...
function error(value:unknown){if(value instanceof ZodError)return json({error:'Solicitud administrativa inválida.'},400);if(value instanceof AuthError||value instanceof FirebaseError)return json({error:value.message},value.status);console.error('Admin request failed');return json({error:'No se pudo completar la operación administrativa.'},503)}
...
export async function POST(request:Request){if(!isAllowedOrigin(request))return json({error:'Origen no permitido.'},403);try{const repo=repository();const admin=await repo.requireAdmin(request);const input=command.parse(await request.json()),firebaseAuth=firebase();if(input.action==='status'){const identity=await repo.firebaseIdentity(input.userId);if(firebaseAuth&&identity?.firebaseUid)await firebaseAuth.updateUser(identity.firebaseUid,{disabled:input.status!=='active'});await repo.setStatus(admin.id,input.userId,input.status);return json({ok:true})}if(input.action==='verify-email'){const identity=await repo.firebaseIdentity(input.userId);if(firebaseAuth&&identity?.firebaseUid)await firebaseAuth.updateUser(identity.firebaseUid,{emailVerified:true});await repo.markEmailVerified(admin.id,input.userId);return json({ok:true})}if(firebaseAuth){const reset=(await repo.listResets()).find(item=>item.id===input.requestId);if(!reset)throw new AuthError('La solicitud ya fue atendida.',409);const link=await firebaseAuth.generatePasswordResetLink(reset.email);await repo.markResetIssued(admin.id,input.requestId);return json({resetLink:link,expiresInMinutes:30})}const token=await repo.issueReset(admin.id,input.requestId);return json({token,expiresInMinutes:30})}catch(value){return error(value)}}
